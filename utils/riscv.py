...
class Riscv:
    ...
    class Branch(TACInstr):
        ...
        def __str__(self) -> str:
            return "beq " + Riscv.FMT3.format(
                str(Riscv.ZERO), str(self.srcs[0]), str(self.target)
            )
    ...
    class NativeLoadWord(NativeInstr):
        def __init__(self, dst: Reg, base: Reg, offset: int, tag=False) -> None:
    ...
    class Call(TACInstr):
        def __init__(self, label: FuncLabel, arglist: [Temp]) -> None:
            super().__init__(InstrKind.Call, [], arglist, label)

        def __str__(self) -> str:
            return "call " + self.label.name

    class La(TACInstr):
        def __init__(self, dst, name: str) -> None:
            super().__init__(InstrKind.SEQ, [dst], [], None)
            self.name = name

        def __str__(self) -> str:
            return "la " + str(self.dsts[0]) + ", " + self.name

    class Load(TACInstr):
        def __init__(self, dst, src, offset: int) -> None:
            super().__init__(InstrKind.SEQ, [dst], [src], None)
            self.offset = offset
    ...
    class Store(TACInstr):
        ...
        def __str__(self) -> str:
            return "sw " + Riscv.FMT_OFFSET.format(
                str(self.srcs[0]), str(self.offset), str(self.dsts[0])
            )
