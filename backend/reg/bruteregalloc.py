...
class BruteRegAlloc(RegAlloc):
    ...
    def localAlloc(self, bb: BasicBlock, subEmitter: SubroutineEmitter):
        ...
        if bb.kind is BlockKind.END_BY_CALL:
            call = bb.locs[len(bb.locs) - 1]

            for src in call.instr.srcs:
                reg = self.allocRegFor(src, True, call.liveIn, subEmitter)
                self.bind(src, reg)

            subEmitter.emitNative(Riscv.SPAdd(-4 * len(call.instr.srcs)))

            for order, src in enumerate(call.instr.srcs):
                subEmitter.emitNative(
                    Riscv.NativeStoreWord(self.bindings[src.index], Riscv.SP, 4 * order)
                )
